...
def smart_walk_generation(G_Undirected, G_Multi, id_to_chord, genre, i = ""):
    ...
    #instantiate song graph
    G_Multi_i = snap.TNEANet.New() #song graph i
    ...
    while len(chord_progression) < MAX_SONG_LENGTH:
        ...
        if not G_Multi_i.IsNode(dstID):
            G_Multi_i.AddNode(dstID)

        G_Multi_i.AddEdge(nodeID, dstID)
        node = G_Multi.GetNI(dstID)
    ...
    snap.SaveEdgeList(G_Multi_i, file_name)
